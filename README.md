# Silicon-Carbon Anode Literature

This private research workspace supports a structured literature review of silicon-carbon negative-electrode materials for traction batteries. It combines reproducible search, candidate screening, PDF acquisition, evidence-grounded deep reading, BibTeX management, and research synthesis. The browser workbench is the ordinary interface, while `battery_lit` is the deterministic command layer.
...
- Deep paper reading is a topic-local skill workflow: the Codex operation worker writes `papers/<bibkey>/deep_read.json`; `battery_lit` only parses, validates, rebuilds notes, and renders reports.

Quick start:

```bash
python3 -m pip install -r requirements.txt -r requirements-dev.txt
bin/battery_lit start --base-dir /tmp --host 0.0.0.0 --port 10005
```
...
`battery_lit start` disables the Codex runtime sandbox by default for this serverlet process because Docker user-namespace sandboxing can block `battery_lit` itself. Topic `policy.yml` and the `battery_lit` CLI remain the project safety boundary. Use `--codex-sandbox` only when debugging in an environment where Codex workspace sandboxing is known to work.

If you have a title and a parent directory but not a final folder name, let the CLI derive a safe slug:

```bash
bin/battery_lit init --base-dir /tmp --title "Silicon-Carbon Anodes for Traction Batteries" --direction "Materials, interfaces, manufacturing, degradation, safety, and cell-level validation of silicon-carbon negative electrodes"
# creates /tmp/silicon-carbon-anodes-for-traction-batteries
```

Running from source:

- Prefer `bin/battery_lit ...`; the wrapper automatically adds this repository's `src/` directory to `PYTHONPATH`.
- If invoking Python modules directly from the project root, set the project-local source path explicitly:

```bash
PYTHONPATH=src python -m battery_lit.cli status --root /tmp/battery-topic --json
```
...
- Create the topic in the browser. The init job uses `templates/skills/topic_init/SKILL.md` and `battery_lit init --base-dir`; it must not inspect sibling topic folders.
...
- By default, `start` runs Codex turns without the Codex runtime sandbox so browser actions can execute `battery_lit` inside Docker. Add `--codex-sandbox` only to opt back into Codex workspace sandboxing.
...
```text
@/<path-to-battery-research-literature>/README.md
使用 battery_lit 工具，在 <parent-dir>/ 下初始化一个新的 topic 目录。
名字定为 "<topic title>"。
...
```text
@/<path-to-battery-research-literature>/README.md
使用 battery_lit 工具，给我在 /paper_hub/ 下初始化一个新的 topic 目录。
名字定为 "动力电池硅碳负极"。
检索方向是收集硅碳负极的材料结构、预锂化、粘结剂与电解液、电极工程、衰减与安全、规模制造和全电池验证研究。
```

If the project is mounted at `/battery_research_literature`, the first line becomes:

```text
@/battery_research_literature/README.md
```

- Prefer explicitly referencing the project README with `@.../README.md`. Do not ask Codex to "load the battery_lit agent"; that wording can be confused with a multi-agent delegation instead of this repository workflow.
- To initialize a topic, ask Codex to use `templates/skills/topic_init/SKILL.md`. If title or direction is missing, Codex should ask for it before running `battery_lit init`.
...
```bash
bin/battery_lit read --root /tmp/battery-topic <bibkey> --parse-only
```
...
Then ask a Codex worker or advanced interactive Codex session to follow `/tmp/battery-topic/skills/paper_deep_read/SKILL.md`, write `/tmp/battery-topic/papers/<bibkey>/deep_read.json`, and run:

```bash
bin/battery_lit read --root /tmp/battery-topic <bibkey> --validate-report
bin/battery_lit read --root /tmp/battery-topic <bibkey> --rebuild-note
```

For an explicit reread that should overwrite stale results without using old notes as evidence, ask Codex to follow `/tmp/battery-topic/skills/paper_reread/SKILL.md`. A compact prompt is:
...
For multiple selected papers, the controlled path is `battery_lit read-many`: one reader session and one independent reviewer session per paper, with final writes performed by the project controller after validation.

To import one already-library paper from an explicitly named source topic, use its source bibkey:

```bash
bin/battery_lit library import-from-topic --root /tmp/target-topic --source-root /tmp/source-topic --source-bibkey Example2026A --json
```
...
State-changing commands refresh static reports automatically. Run `bin/battery_lit html build --root /tmp/battery-topic` manually only after direct file edits, report repair work, or when `BATTERY_LIT_AUTO_HTML=0` was used.
...
- Use `./bin/battery_lit start --base-dir <parent-dir>` for ordinary first-time operation, or `./bin/battery_lit start --root <topic>` for an existing topic. `./bin/battery_lit web serve` is a compatibility alias for existing topics.
