...
```bash
export BATTERY_LIT_CODEX_MODEL=gpt-5.6-sol
export BATTERY_LIT_CODEX_EFFORT=medium
```
...
```bash
./bin/battery_lit start --base-dir /paper_hub --host 0.0.0.0 --port 10005
```

Create the topic in the browser. The init job uses Codex plus `battery_lit init --base-dir` and must not inspect sibling topic folders. If Codex, the selected model/effort, or the CLI fails, the Create Topic page shows the error in Recent Jobs.

For scripts or debugging, use the CLI directly:

```bash
./bin/battery_lit init --base-dir /paper_hub --title "<topic title>" --direction "<one paragraph research direction>"
```
...
The tool should use `bin/battery_lit init --base-dir <parent-dir> --title "<title>" --direction "<direction>"` and should not inspect sibling topic folders.
...
By default, `battery_lit start` disables the Codex runtime sandbox for the serverlet process. This is intentional for Docker deployments where Codex workspace sandboxing can fail before `battery_lit` starts. Topic `policy.yml` and the CLI remain the safety boundary. Use `--codex-sandbox` only when you explicitly want to test Codex workspace sandboxing.
...
```bash
python3 -m pytest -q
./bin/battery_lit policy check --root <topic> --json
./bin/battery_lit status --root <topic> --json
...
BATTERY_LIT_LIVE_CODEX=1 python3 scripts/run_live_codex_probe.py --root <topic>
BATTERY_LIT_LIVE_CODEX=1 python3 scripts/run_live_web_flow_probe.py --model gpt-5.6-sol --effort medium
```
...
- The Codex operator calls `battery_lit` for state-changing work.
...
- Bootstrap or compatibility job stuck: inspect `.battery/active_job.json`, `.battery/jobs/<job_id>/events.jsonl`, `.battery/jobs/<job_id>/stderr.log`, and `.battery/jobs/<job_id>/summary.json`.
- Model or effort unsupported: choose Account default/default in the browser, or unset `BATTERY_LIT_CODEX_MODEL` and `BATTERY_LIT_CODEX_EFFORT`.
...
- Stale state: run `battery_lit policy check --root <topic> --json` and `battery_lit status --root <topic> --json`.
