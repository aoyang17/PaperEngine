...
SERVERLET_OPERATION_PROMPT = """You are the battery_lit operation worker for one literature-management job.
...
Operating rules:
- Use battery_lit CLI commands for state changes whenever a command exists.
- Use `battery_lit` CLI commands for all topic state changes when a command exists.
- When a command says `battery_lit`, run `{project_root}/bin/battery_lit`.
...
- Do not run arbitrary nested Codex/Claude/LLM CLI processes. The only allowed model-backed tool exceptions are `battery_lit read <bibkey> --vision-formulas` and `battery_lit read-many ...`, both of which run through project-controlled CLI runners with bounded artifact contracts.
...
- Remove a unique candidate queue item by bibkey -> use `battery_lit candidates remove-by-bibkey <bibkey>`; this is queue-only and must not delete `library.bib`, `papers/<bibkey>/`, PDFs, notes, or reading HTML. If multiple queue items match the same bibkey, stop and run dedup or ask for a specific candidate ID instead of deleting them all.
- Update library metadata or rename a bibkey -> use `battery_lit library update-metadata <bibkey> --metadata <file> [--new-bibkey <key>]`; metadata must come from real search/source results that were checked repeatedly by the agent, never from model memory or invention. DOI/arXiv are preferred; OpenAlex Work ID, Semantic Scholar ID, DBLP key, or a verified publisher/work URL may be used for papers that lack DOI/arXiv. ISSN/ISBN alone are not enough. The command marks the BibTeX entry as unverified because battery_lit did not independently verify the supplied metadata.
- Download PDF / acquire / promote -> use `battery_lit acquire`, `battery_lit promote`, then `battery_lit bib check` and `battery_lit pdf check`.
...
def build_bootstrap_init_prompt(
    ...
    return f"""You are the battery_lit bootstrap worker for creating one new literature topic.
...
def collect_candidates_task(target_new: int = 20, score_threshold: float | None = None, query: str | None = None) -> str:
    ...
    command = f"battery_lit collect --target-new {target_new}"
...
def mark_candidate_task(candidate_id: str, decision: str) -> str:
    return (
        ...
        f"Use `battery_lit candidates mark {candidate_id} {decision}` from the active topic root.\n"
...
def acquire_candidate_task(candidate_id: str | Iterable[str]) -> str:
    ...
    command_text = (
        f"Use `battery_lit acquire {candidate_id}` first. If it succeeds, use `battery_lit promote {candidate_id}`.\n"
        if isinstance(candidate_id, str)
        else "\n".join(
            f"Use `battery_lit acquire {candidate}` first. If it succeeds, use `battery_lit promote {candidate}`."
    ...
    return (
        ...
        "Run `battery_lit bib check` and `battery_lit pdf check` after promotion."
    )
...
def read_paper_task(bibkey: str | Iterable[str]) -> str:
    ...
    if not isinstance(bibkey, str):
        ...
        return (
            ...
            f"Run `battery_lit read-many {key_args} --force-reread --json` from the active topic root.\n"
            ...
            "Do not start arbitrary nested Codex processes for multi-paper reading; use only the controlled `battery_lit read-many` runner.\n"
        )

    def command_for(key: str) -> str:
        return (
            f"For `{key}`, first run `battery_lit read {key} --validate-report`, then run `battery_lit read {key} --rebuild-note`, then run `battery_lit read {key} --quality-audit` if validation and rebuild pass.\n"
            f"If validation, rebuild, and quality audit all pass, skip `{key}`: do not run `battery_lit read {key} --parse-only`, "
            f"do not run `battery_lit read {key} --rebuild-note`, and do not rewrite "
            ...
            f"If the user explicitly asked to re-read, reinterpret, refresh, or fix stale reading knowledge for `{key}`, run `battery_lit read {key} --parse-only` before writing the bundle even if a previous report exists.\n"
            f"If validation fails because the reading bundle is missing, invalid, stale, or missing `math_index.json`, run `battery_lit read {key} --parse-only`.\n"
            ...
            f"`vision_fallback.needed`, run `battery_lit read {key} --vision-formulas` before writing the bundle. "
            ...
            f"After writing the bundle, run `battery_lit read {key} --validate-report`, `battery_lit read {key} --rebuild-note`, and `battery_lit read {key} --quality-audit`."
        )
    ...
    return (
        ...
        "If the user explicitly asks only to rebuild notes or HTML without re-reading, run `battery_lit read` with `--rebuild-note` for that named paper instead of re-reading it.\n"
        "`read --rebuild-note` refreshes static HTML automatically after a newly written or explicitly rebuilt report.\n"
        "After any multi-paper read job completes all requested chunks, run `battery_lit tool audit-readings --json` and fix repeated/template reader-facing text before reporting completion.\n"
        "Do not load, compare, or discuss topic-local copies of the paper_deep_read skill for this serverlet action.\n"
        "Keep visible progress sparse: do not narrate instruction loading, sandbox retry, preflight checks, parser setup, or validator retries unless they block completion.\n"
...
def dismiss_candidate_task(candidate_id: str) -> str:
    return (
        ...
        f"Use `battery_lit candidates dismiss {candidate_id}` from the active topic root.\n"
...
def html_build_task(target: str | None = None) -> str:
    ...
    return (
        ...
        "Use `battery_lit html build` from the active topic root, then run `battery_lit status --json`."
    )
...
def health_check_task() -> str:
    return (
        ...
        "Use `battery_lit policy check`, `battery_lit status --json`, `battery_lit bib check`, and `battery_lit pdf check`.\n"
...
def chat_task(message: str) -> str:
    return (
        "Answer this bounded user request using the topic guide and battery_lit summaries.\n"
...
def session_action_task(action: str, payload: dict[str, object]) -> str:
    if action == "search_30":
        ...
        task = "Check the topic BibTeX library.\nUse `battery_lit bib check` from the active topic root."
        ...
        task = (
            f"Handle browser action `{action}` with this payload using the closest safe battery_lit workflow:\n"
