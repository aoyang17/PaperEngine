...
def finalize_read_batch(root: str | Path, run_id: str, *, rebuild: bool = True) -> dict[str, Any]:
    ...
    try:
        ...
        verification.append(f"battery_lit tool audit-readings --selected-batch: {'pass' if batch_result.get('ok') else 'fail'}")
    ...
    return {
        ...
        "library_audit": {
            "ok": None,
            "skipped": True,
            "reason": "run `battery_lit tool audit-readings --json` after all read-batch chunks are finalized",
        },
    }
...
def _run_one_draft_worker(
    ...
    *,
    ...
    job_dir = root / ".battery" / "jobs" / f"{run_id}-{bibkey}-draft-worker"
    ...
    event_iter = (
        runner.run_until_outputs(
            ...
            job_dir,
            required_outputs,
            stable_seconds=float(os.environ.get("BATTERY_LIT_READ_BATCH_OUTPUT_STABLE_SECONDS", "5")),
            timeout_seconds=float(os.environ.get("BATTERY_LIT_READ_BATCH_WORKER_TIMEOUT", str(DEFAULT_READ_BATCH_WORKER_TIMEOUT_SECONDS))),
...
"""
    return f"""You are a battery_lit paper-reading draft worker for exactly one paper.
...
- After the three draft JSON files are syntactically written, immediately write {rel_output}. Do not run `battery_lit read ... --validate-report`, `--quality-audit`, `--rebuild-note`, or `read-batch --finalize`; the parent batch finalizer owns validation and repair.
...
- Do not inspect project implementation source such as `src/battery_lit/*.py`, renderer code, validator code, or test files to predict hidden checks. If a schema detail is unclear, make the best contract-shaped draft and let the parent finalizer report the concrete validation error.
...
def _run_one_harvest(root: Path, run_id: str, findings_dir: Path, bibkey: str, runner: Any) -> dict[str, Any]:
    job_dir = root / ".battery" / "jobs" / f"{run_id}-{bibkey}-harvest"
    job_dir.mkdir(parents=True, exist_ok=True)
...
def _build_harvest_prompt(root: Path, bibkey: str, output_path: Path) -> str:
    rel_output = output_path.relative_to(root)
    return f"""You are a read-only battery_lit evidence-harvest sidecar for one paper.
