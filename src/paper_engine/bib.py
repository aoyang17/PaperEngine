...
def format_bibtex(meta: dict[str, Any]) -> str:
    ...
    if meta.get("metadata_status"):
        fields.append(("paperEngineMetadataStatus", meta["metadata_status"]))
    if meta.get("verified_sources"):
        fields.append(("paperEngineVerifiedSource", "; ".join(str(item) for item in meta["verified_sources"] if item)))
    if meta.get("metadata_source_note"):
        fields.append(("paperEngineMetadataNote", meta["metadata_source_note"]))
...
def _normalize_update_metadata(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    meta = {
        ...
        "metadata_source_note": str(
            ...
            or "user_or_agent_supplied_metadata_not_verified_by_paper_engine"
        ),
    }
