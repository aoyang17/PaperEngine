...
def _validate_serverlet_topic(root: str) -> None:
    ...
    if missing:
        raise FileNotFoundError(
            ...
            "Run `paper_engine init` before `paper_engine start`."
        )


def _auto_build_html(root: str) -> None:
    if str(os.environ.get("PAPER_ENGINE_AUTO_HTML", "")).strip().lower() in {"0", "false", "no"}:
        return
    try:
...
def _configure_start_codex_sandbox(use_codex_sandbox: bool) -> None:
    if use_codex_sandbox:
        os.environ.pop("PAPER_ENGINE_CODEX_BYPASS_SANDBOX", None)
        print(
            "Codex workspace sandbox enabled; this may fail in Docker containers without user namespaces.",
            file=sys.stderr,
        )
        return
    os.environ["PAPER_ENGINE_CODEX_BYPASS_SANDBOX"] = "1"
    print(
        "Codex sandbox disabled for this serverlet; paper_engine policy and CLI remain the safety boundary.",
        file=sys.stderr,
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="paper_engine")
    ...
    p_start.add_argument(
        ...
        help=(
            ...
            "because Docker user-namespace restrictions can block paper_engine itself."
        ),
    )
    ...
    p_one_page = sub.add_parser(
        "one-page",
        description="Prepare or render an evidence-backed, standalone one-page paper reading.",
    )
    opsub = p_one_page.add_subparsers(dest="one_page_command", required=True)
    p_op_prepare = opsub.add_parser("prepare")
    p_op_prepare.add_argument("pdf")
    p_op_prepare.add_argument("--work-dir", required=True)
    p_op_prepare.add_argument("--json", action="store_true")
    p_op_build = opsub.add_parser("build")
    p_op_build.add_argument("pdf")
    p_op_build.add_argument("--content", required=True)
    p_op_build.add_argument("--output", required=True)
    p_op_build.add_argument("--json", action="store_true")
    ...
    from .simulation_reproduction.cli import add_parser as add_reproduction_parser

    add_reproduction_parser(sub)
...
def run(args: argparse.Namespace) -> int:
    if args.command == "init":
        ...
        if not args.title:
            raise ValueError("paper_engine init requires --title; ask the user; do not infer from filesystem")
        if not args.direction:
            raise ValueError("paper_engine init requires --direction; ask the user; do not infer from filesystem")
    ...
    if args.command == "start":
        ...
        if args.base_dir and args.root is not None:
            raise ValueError("--root and --base-dir are mutually exclusive for paper_engine start")
        if args.base_dir:
    ...
    if args.command == "one-page":
        from .one_page import build_one_page, prepare_one_page

        if args.one_page_command == "prepare":
            result = prepare_one_page(args.pdf, args.work_dir)
        else:
            result = build_one_page(args.pdf, args.content, args.output)
        emit(result, as_json=args.json)
        return 0 if result["ok"] else 1

    if args.command == "reproduce":
        from .simulation_reproduction.cli import run as run_reproduction

        code, result = run_reproduction(args)
        emit(result)
        return code
