...
class SubprocessFormulaVisionRunner:
    def __init__(self, codex_bin: str | None = None, model: str | None = None, effort: str | None = None) -> None:
        ...
        self.model = model or os.environ.get("PAPER_ENGINE_CODEX_MODEL")
        self.effort = effort or os.environ.get("PAPER_ENGINE_CODEX_EFFORT") or "medium"
...
def transcribe_formulas(
    ...
    *,
    ...
    if not math_index_path.exists():
        return {"ok": False, "errors": [f"missing {MATH_INDEX_NAME} for {bibkey}; run `paper_engine read {bibkey} --parse-only` first"]}
    try:
...
def _prompt_for_batch(root: Path, bibkey: str, images: list[Path]) -> str:
    ...
    return (
        "You are a bounded formula-vision transcriber for paper_engine.\n"
