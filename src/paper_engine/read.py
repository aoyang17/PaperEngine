...
def _parse_pdf_text(pdf_path: Path) -> tuple[str, str, str | None]:
    backend = os.environ.get("PAPER_ENGINE_PARSER_BACKEND", "pymupdf4llm").strip().lower() or "pymupdf4llm"
    if backend == "marker":
...
def _parse_with_marker(pdf_path: Path) -> tuple[str | None, str | None]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="paper-engine-marker-") as tmp:
...
def _build_math_index(
    ...
    return {
        ...
        "vision_fallback": {
            ...
            "instruction": (
                "Run `paper_engine read <bibkey> --vision-formulas` when parsed text cannot safely recover key "
...
def _validate_reading_contract(
    ...
    if "theory" in set(lenses + report_lenses) and math_index is None:
        errors.append(f"missing {MATH_INDEX_NAME} for active theory lens; rerun `paper_engine read <bibkey> --parse-only`")
    if "theory" in set(lenses + report_lenses) and _math_vision_fallback_pending(math_index):
        errors.append(f"{MATH_INDEX_NAME} vision_fallback is pending; run `paper_engine read <bibkey> --vision-formulas` before final writing")
    for lens in lenses:
