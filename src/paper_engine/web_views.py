...
def _job_state_dir(root: str | Path, state_dir: str | Path | None = None) -> Path:
    return Path(state_dir).expanduser().resolve() if state_dir else TopicPaths.from_root(root).root / ".paper_engine"
...
def codex_sandbox_warning() -> str:
    if os.environ.get("PAPER_ENGINE_HIDE_SANDBOX_WARNING"):
        return ""
