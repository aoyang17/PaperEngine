...
from paper_engine.codex_worker import SubprocessCodexRunner
...
def test_start_command_calls_serve_web_and_disables_codex_sandbox_by_default(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("paper_engine.web_app.serve_web", fake_serve_web)
    ...
    assert os.environ["PAPER_ENGINE_CODEX_BYPASS_SANDBOX"] == "1"
...
def test_start_can_opt_back_into_codex_workspace_sandbox(tmp_path, monkeypatch, capsys):
    ...
    assert "PAPER_ENGINE_CODEX_BYPASS_SANDBOX" not in os.environ
...
def test_start_rejects_missing_topic_files(tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert result == 1
    assert "paper_engine init" in captured.err
    assert "topic.yml" in captured.err


def test_web_serve_remains_supported():
    proc = subprocess.run(
        [str(ROOT / "bin" / "paper_engine"), "web", "serve", "--help"],
        text=True,
        capture_output=True,
        check=True,
    )
...
def test_read_batch_draft_workers_can_prepare_and_run_in_one_cli_call(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("paper_engine.read_batch.run_read_batch_draft_workers", fake_run)
