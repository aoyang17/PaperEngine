...
from battery_lit.codex_worker import CodexEvent, FakeCodexRunner, SubprocessCodexRunner
from battery_lit.jobs import JobAlreadyActive, JobManager
from battery_lit.prompt_contracts import build_worker_prompt
...
def test_worker_prompt_contains_required_context_and_boundaries(tmp_path):
    ...
    prompt = build_worker_prompt(
        project_root=Path("/project/battery"),
        topic_root=tmp_path,
    ...
    assert "Use battery_lit CLI commands for state changes" in prompt
...
def test_job_manager_persists_successful_job(tmp_path):
    ...
    manager = JobManager(tmp_path, runner=runner, project_root=Path("/project/battery"))

    result = manager.run_job("Collect papers", action="collect")

    job_dir = tmp_path / ".battery" / "jobs" / result["job_id"]
    ...
    assert not (tmp_path / ".battery" / "active_job.json").exists()
    ...
    assert json.loads((tmp_path / ".battery" / "jobs.jsonl").read_text().splitlines()[0])["job_id"] == result["job_id"]
...
def test_job_manager_rejects_when_active_job_exists(tmp_path):
    ...
    active = tmp_path / ".battery" / "active_job.json"
    active.parent.mkdir()
    active.write_text(json.dumps({"job_id": "existing"}), encoding="utf-8")

    manager = JobManager(tmp_path, runner=FakeCodexRunner([]), project_root=Path("/project/battery"))
...
def test_job_manager_atomic_lock_allows_only_one_concurrent_job(tmp_path):
    ...
    first = JobManager(tmp_path, runner=runner, project_root=Path("/project/battery"))
    second = JobManager(tmp_path, runner=FakeCodexRunner([]), project_root=Path("/project/battery"))
    ...
    runner.release.set()
    for _ in range(100):
        if not (tmp_path / ".battery" / "active_job.json").exists():
            break
        time.sleep(0.02)
    assert not (tmp_path / ".battery" / "active_job.json").exists()
...
def test_subprocess_runner_uses_account_default_and_env_model(monkeypatch):
    monkeypatch.delenv("BATTERY_LIT_CODEX_MODEL", raising=False)
    monkeypatch.delenv("BATTERY_LIT_CODEX_EFFORT", raising=False)
    assert SubprocessCodexRunner().model is None
    assert SubprocessCodexRunner().effort is None

    monkeypatch.setenv("BATTERY_LIT_CODEX_MODEL", "gpt-5.6-sol")
    monkeypatch.setenv("BATTERY_LIT_CODEX_EFFORT", "xhigh")
...
def test_subprocess_runner_command_matches_current_codex_cli(monkeypatch, tmp_path):
    monkeypatch.delenv("BATTERY_LIT_CODEX_MODEL", raising=False)
    monkeypatch.delenv("BATTERY_LIT_CODEX_BIN", raising=False)
...
def test_subprocess_runner_uses_explicit_codex_bin_env(monkeypatch, tmp_path):
    monkeypatch.setenv("BATTERY_LIT_CODEX_BIN", "/opt/codex/bin/codex")
...
def test_subprocess_runner_merges_stderr_to_stdout(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("battery_lit.codex_worker.subprocess.Popen", fake_popen)
