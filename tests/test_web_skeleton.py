...
def test_web_dashboard_shows_active_job(tmp_path):
    ...
    state_dir = tmp_path / ".paper_engine"
    state_dir.mkdir()
...
def test_dashboard_collect_action_enqueues_codex_job(tmp_path):
    ...
    assert (tmp_path / ".paper_engine" / "jobs" / data["job_id"] / "prompt.txt").exists()
    ...
    assert "paper_engine collect" in prompt
...
def test_candidate_mark_action_enqueues_codex_job_without_direct_mutation(tmp_path):
    ...
    assert "paper_engine candidates mark CAND-001 relevant" in _job_prompt(tmp_path, data["job_id"])
...
def test_candidate_acquire_action_enqueues_codex_job(tmp_path):
    ...
    prompt = _job_prompt(tmp_path, data["job_id"])
    assert "paper_engine acquire CAND-001" in prompt
    assert "paper_engine promote CAND-001" in prompt
